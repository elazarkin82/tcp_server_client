...
public:
	...
	public:
		...
		uint64_t id();
		uint32_t cache_size();
		uint32_t get_connection_cache_size();
		void set_connection_cache_size(uint32_t);
	...
	public:
		...
		virtual void onRequestToRepeat(Client *client, void *data, uint32_t size) = 0;
	};
private:
...
public:
    ...
    void send(Client *client, void *data, uint32_t size);
    void close();
...
private:
    void onClientReceiveCallbackWrapper(Client *client, void *data, uint32_t size);
    void ping_checker_thread();
    uint32_t get_used_cache_size();
};
